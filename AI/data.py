...
#         train, test, val = self.split_train_test()[2]
...
#         h, w, _ = target_shape
#         img = load_img(image, target_size=(h, w))
...
class DataSet():
    ...
    def get_all_sequences_in_memory(self, train_test_val, data_type):
        ...
        for row in data:
           if data_type == 'images':
                print("just checking that we don't go in this code")
                frames = self.get_frames_for_sample(row)
                frames = self.rescale_list(frames, self.seq_length)
               ...
               if sequence is None:
                   ...
                   sequence = np.reshape(sequence, (self.seq_length, -1))
                   X.append(sequence)
                   y.append(self.get_class_one_hot(row[1]))
    ...
    def get_extracted_sequence(self, data_type, sample):
        """Get the saved extracted features."""
        filename = sample[2]
        path = os.path.join(self.data_path, 'sequences', sample[1], filename + '-' + str(self.seq_length) +
                            '-' + data_type + '.npy')
        print(path)
        if os.path.isfile(path):
            return np.load(path)
